...
// --- Moderacja: prosty panel logowania (tylko localStorage)
...
document.getElementById('admin-login-btn').onclick = async function() {
    ...
    if (username !== 'AxonicAdmin') {
        errorDiv.textContent = "Nieprawidłowy login";
        return;
    }
    if (password !== 'Galaxy2025!') {
...
// --- Generowanie pliku JSON z grą (do katalogu "games/")
document.getElementById('add-game-btn').onclick = async function() {
    ...
    // Tworzenie i pobieranie pliku JSON (do katalogu games/)
    ...
    statusDiv.textContent = "Plik gry wygenerowany! Dodaj go do katalogu games/ w repozytorium.";
...
// --- Wyszukiwanie i lista gier (ładuj z games/index.json)
let games = {};
...
async function loadGames() {
    try {
        ...
        document.getElementById('game-list').innerHTML = "<div style='color:#ff3263;font-weight:700;'>Nie można pobrać listy gier.</div>";
    }
}
